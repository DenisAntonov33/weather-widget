...
if (!API_KEY) {
  console.warn(
    'OPENWEATHER_API_KEY is not set. Please create a .env file with your OpenWeatherMap API key.'
  );
}
...
function calculateDewPoint(temp: number, humidity: number): number {
  ...
  const gamma =
    Math.log(humidity / 100.0) + (MAGNUS_COEFFICIENT_B * temp) / (MAGNUS_COEFFICIENT_C + temp);
